...
class ActorCritic(nn.Module):
    ...
    @staticmethod
    def update_ac(network, rewards, log_probs, values, masks, Qval, gamma=GAMMA, entropy_beta=0.001):
        ...
        with torch.no_grad():
            states = [value.detach() for value in values]

        # Use network.forward to get fresh probabilities (policy) for entropy computation
        with torch.no_grad():
            probs_list = []
            for val in values:
                p, _ = network.forward(val.unsqueeze(0))
                probs_list.append(p[0])

            probs_tensor = torch.stack(probs_list)
            entropy = -(probs_tensor * torch.log(probs_tensor + 1e-9)).sum(dim=1).mean()
